...
public:
	...
	struct CBChangesEveryInstanceDrawing
	{
		...
		DirectX::XMFLOAT4 index_P_XYZ;
		DirectX::XMFLOAT4 index_N_XYZ;
	};
...
public:
	...
	ComPtr<ID3D11VertexShader> m_pBlockVS;

	ComPtr<ID3D11PixelShader> m_pBasicPS;
	ComPtr<ID3D11PixelShader> m_pBlockPS;
...
bool BasicEffect::InitAll(ID3D11Device * device)
{
	...
	//绘制方块
	//
	HR(CreateShaderFromFile(L"HLSL\\Block_VS.cso", L"HLSL\\Block_VS.hlsl", "VS", "vs_5_0", blob.ReleaseAndGetAddressOf()));
	HR(device->CreateVertexShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, pImpl->m_pBlockVS.GetAddressOf()));
	// 创建顶点布局
	HR(device->CreateInputLayout(basicInstLayout, ARRAYSIZE(basicInstLayout),
		blob->GetBufferPointer(), blob->GetBufferSize(), pImpl->m_pInstancePosNormalTexLayout.GetAddressOf()));
	
	HR(CreateShaderFromFile(L"HLSL\\Block_PS.cso", L"HLSL\\Block_PS.hlsl", "PS", "ps_5_0", blob.ReleaseAndGetAddressOf()));
	HR(device->CreatePixelShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, pImpl->m_pBlockPS.GetAddressOf()));
	...
	D3D11SetDebugObjectName(pImpl->m_pBlockPS.Get(), "BasicEffect.Block_PS");
	D3D11SetDebugObjectName(pImpl->m_pBlockVS.Get(), "BasicEffect.Block_VS");
...
void BasicEffect::SetRenderDefault(ID3D11DeviceContext * deviceContext, RenderType type)
{
	...
	else if (type == RenderBlockInstance)
	{
		deviceContext->IASetInputLayout(pImpl->m_pInstancePosNormalTexLayout.Get());
		deviceContext->VSSetShader(pImpl->m_pBlockVS.Get(), nullptr, 0);
		deviceContext->PSSetShader(pImpl->m_pBlockPS.Get(), nullptr, 0);
	}
	...
	if (type == RenderBlockInstance)//方块用点采样
		deviceContext->PSSetSamplers(0, 1, RenderStates::SSPointWrap.GetAddressOf());
	else// 使用各向异性过滤获取更好的绘制质量
...
//纹理索引设置
void BasicEffect::SetTexIndex(DirectX::XMFLOAT4 index_p_xyz, DirectX::XMFLOAT4 index_n_xyz)
{
	auto& cBuffer = pImpl->m_CBInstDrawing;
	cBuffer.data.index_P_XYZ = index_p_xyz;
	cBuffer.data.index_N_XYZ = index_n_xyz;
	pImpl->m_IsDirty = cBuffer.isDirty = true;
}
...
void BasicEffect::Apply(ID3D11DeviceContext * deviceContext)
{
	...
	deviceContext->PSSetShaderResources(1, 1, pImpl->m_pTextureCube.GetAddressOf());//天空盒纹理
	deviceContext->PSSetShaderResources(2, 1, pImpl->m_pTextures.GetAddressOf());//方块纹理
