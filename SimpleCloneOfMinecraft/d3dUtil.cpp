...
//记录不同方块的box数据
UINT BlockBoxs[BlockTypeCount][6][4]=
{
	...
	//草方块
	{{3,4,0,1},{3,4,0,1},{0,1,0,1},{2,3,0,1},{3,4,0,1},{3,4,0,1}},
	//石头
	{{1,2,0,1},{1,2,0,1},{1,2,0,1},{1,2,0,1},{1,2,0,1},{1,2,0,1}},
	//木板
	{{4,5,0,1},{4,5,0,1},{4,5,0,1},{4,5,0,1},{4,5,0,1},{4,5,0,1}},
	//木头
	{{4,5,1,2},{4,5,1,2},{5,6,1,2},{5,6,1,2},{4,5,1,2},{4,5,1,2}},
	//树叶
	{{5,6,3,4},{5,6,3,4},{5,6,3,4},{5,6,3,4},{5,6,3,4},{5,6,3,4}},
	//基岩
	{{1,2,1,2},{1,2,1,2},{1,2,1,2},{1,2,1,2},{1,2,1,2},{1,2,1,2}}
};

//创建方块纹理数组
HRESULT CreateBlockTextureArrayFromFile(
	ID3D11Device* d3dDevice,
	ID3D11DeviceContext* d3dDeviceContext,
	const std::wstring& BlockTexturesFileName,
	ID3D11Texture2D** BlockTextureArray,
	ID3D11ShaderResourceView** BlocktextureArrayView,
	bool generateMips
	)
{
	// 检查设备、文件名数组是否非空
	if (!d3dDevice || BlockTexturesFileName.empty())
		return E_INVALIDARG;

	//HRESULT hr;
	UINT arraySize = BlockTypeCount * 6;//每种方块占六个纹理元素

	//读取方块纹理
	ID3D11Texture2D* BlockTextures = nullptr;
	ID3D11ShaderResourceView* BlockTexturesSRV = nullptr;

	//从文件中读取方块纹理，作为GPU拷贝源
	HRESULT hResult = CreateWICTextureFromFile(d3dDevice,
		(generateMips ? d3dDeviceContext : nullptr),
		BlockTexturesFileName.c_str(),
		(ID3D11Resource**)&BlockTextures,
		(generateMips ? &BlockTexturesSRV : nullptr));
	...
	//创建纹理描述
	D3D11_TEXTURE2D_DESC texDesc, texArrayDesc;
	BlockTextures->GetDesc(&texDesc);

	//新建纹理数组的desc
	UINT squareLength = texDesc.Width / 16;//导入的是256*256的纹理，每一个方块的边长是16
	texArrayDesc.Width = squareLength;
	texArrayDesc.Height = squareLength;
	texArrayDesc.MipLevels = (generateMips ? texDesc.MipLevels - 4 : 1);
	texArrayDesc.ArraySize = arraySize;
	texArrayDesc.Format = texDesc.Format;
	texArrayDesc.SampleDesc.Count = 1;		// 不能使用多重采样
	texArrayDesc.SampleDesc.Quality = 0;
	...
	//创建纹理数组
	ID3D11Texture2D* pTexArray = nullptr;
	hResult = d3dDevice->CreateTexture2D(&texArrayDesc, nullptr, &pTexArray);
	if (FAILED(hResult))
	{
		SAFE_RELEASE(BlockTextures);
		SAFE_RELEASE(BlockTexturesSRV);
		return hResult;
	}
	...
	for (UINT i = 0; i < BlockTypeCount; ++i)//遍历不同方块
	{
		for (UINT j = 0; j < texArrayDesc.MipLevels; ++j)//遍历每个纹理元素的子资源
		{
			for (UINT k = 0; k < 6; ++k)//拷贝6个面
			{
				box.left = squareLength * BlockBoxs[i][k][0];
				box.right = squareLength * BlockBoxs[i][k][1];
				box.top = squareLength * BlockBoxs[i][k][2];
				box.bottom = squareLength * BlockBoxs[i][k][3];
				d3dDeviceContext->CopySubresourceRegion(
					pTexArray,
					D3D10CalcSubresource(j, 6 * i + k, texArrayDesc.MipLevels),
					0, 0, 0,
					BlockTextures,
					j,
					&box);
			}
			//下一个miplevels纹理宽高为原来的1/2
			//squareLength /= 2;
		}
	}

	//必要时创建着色器资源视图
	if (BlocktextureArrayView)
	{
		D3D11_SHADER_RESOURCE_VIEW_DESC viewDesc;
		viewDesc.Format = texArrayDesc.Format;
		viewDesc.ViewDimension= D3D11_SRV_DIMENSION_TEXTURE2DARRAY;
		...
		hResult = d3dDevice->CreateShaderResourceView(pTexArray, &viewDesc, BlocktextureArrayView);
		if (FAILED(hResult))
		{
			SAFE_RELEASE(pTexArray);
			return hResult;
